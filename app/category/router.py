...
def create_category(
    db: Annotated[Session, Depends(get_db)], category_in: CategoryCreate
):
...
def get_by_id_category(db: Annotated[Session, Depends(get_db)], category_id: int):
    return CategoryServices.get_all(db, category_id)


@router.delete("/delete/{category_id}")
def delete_category(db: Annotated[Session, Depends(get_db)], category_id: int):
