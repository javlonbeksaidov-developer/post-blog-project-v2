...
def get_by_id_post(db: Annotated[Session, Depends(get_db)], post_id: int):
    return PostServices.get_by_id(db, post_id)


@router.delete("delete-post/{post_id}/")
def delete_post(db: Annotated[Session, Depends(get_db)], post_id: int):
    return PostServices.delete(db, post_id)


@router.put("/update-post/{post_id}/")
def update_all_post(
    db: Annotated[Session, Depends(get_db)], post_id: int, post_in: PostUpdate
):
...
def update_item_post(
    db: Annotated[Session, Depends(get_db)], _:Annotated[User, Depends(require_author_or_admin)], post_id: int, post_in: PostPatch
):
